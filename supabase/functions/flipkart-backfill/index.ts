...
const fetchInitialPage = async (
  ...
  if (!response.ok) {
    ...
    console.error(
      `[flipkart-backfill] initial fetch failed | status=${response.status} ${response.statusText} | endpoint=${endpoint} | body=${bodyText.slice(0, 1500)}`
    )
  ...
  console.log(
    `[flipkart-backfill] initial fetch success | shipments=${payload.shipments?.length ?? 0} | hasMore=${Boolean(payload.hasMore)}`
  )
...
const fetchNextPage = async (token: string, nextPageUrl: string) => {
  ...
  if (!response.ok) {
    const bodyText = await response.text()
    console.error(
      `[flipkart-backfill] next fetch failed | status=${response.status} ${response.statusText} | endpoint=${endpoint.toString()} | body=${bodyText.slice(0, 1500)}`
    )
  ...
  const payload = await response.json() as FlipkartFilterResponse
  console.log(
    `[flipkart-backfill] next fetch success | shipments=${payload.shipments?.length ?? 0} | hasMore=${Boolean(payload.hasMore)}`
  )
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (apiKeyError) {
      console.error(`[flipkart-backfill] api key query failed | id=${requestId} | error=${apiKeyError.message}`)
      throw apiKeyError
    }
    ...
    for (const filter of FILTERS) {
      const filterShipments = await collectShipmentsForFilter(flipkartToken, filter)
    ...
    console.log(`[flipkart-backfill] fetched unique shipments | count=${shipments.length}`)
    ...
    console.log(`[flipkart-backfill] order upsert attempted | id=${requestId} | rows=${ordersToUpsert.length}`)

    if (orderError) {
      console.error(`[flipkart-backfill] order upsert failed | id=${requestId} | error=${orderError.message}`)
      throw orderError
    }
    ...
    if (shipmentIds.length > 0) {
      ...
      if (cleanupError) {
        console.error(`[flipkart-backfill] items cleanup failed | id=${requestId} | error=${cleanupError.message}`)
        throw cleanupError
      }

      console.log(`[flipkart-backfill] items cleanup complete | id=${requestId} | shipmentCount=${shipmentIds.length}`)
    }
    ...
    if (itemsToUpsert.length > 0) {
      ...
      if (itemError) {
        console.error(`[flipkart-backfill] item upsert failed | id=${requestId} | error=${itemError.message}`)
        throw itemError
      }
    }
        ...
        const geoEnrichPromise = fetch(`${supabaseUrl}/functions/v1/flipkart-geo-enrich`, {
          ...
          .then(async (res) => {
            if (!res.ok) {
              ...
              console.error(`[flipkart-backfill] geo enrich async invoke failed | id=${requestId} | status=${res.status} | body=${body.slice(0, 1000)}`)
            }
          })
          .catch((error) => {
            console.error(`[flipkart-backfill] geo enrich async invoke error | id=${requestId} | error=${String((error as Error)?.message ?? error)}`)
          })

        EdgeRuntime.waitUntil(geoEnrichPromise)
        geoEnrichQueued = true
      } catch (error) {
        console.error(`[flipkart-backfill] geo enrich queue error | id=${requestId} | error=${String((error as Error)?.message ?? error)}`)
      }
    }

    console.log(`[flipkart-backfill] sync complete | id=${requestId} | elapsedMs=${Date.now() - startedAt}`)
    ...
    console.error(
      `[flipkart-backfill] sync failed | id=${requestId} | elapsedMs=${Date.now() - startedAt} | error=${String((err as Error)?.message ?? err)}`
    )
    const stack = (err as Error)?.stack
    if (stack) {
      console.error(`[flipkart-backfill] stack | id=${requestId} | ${stack}`)
    }
